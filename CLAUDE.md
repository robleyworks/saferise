...
| `sr-cp-` | The crisis-pathway banner around the player on `protocol.html` — SR-417. Shown only on t1-06, t1-10, t2-10 (LG-285). Not `.crisis` (that class is the Cue Card banner, a false friend — see the HTML comment above it in `protocol.html`) and not `sr-pp-` (a distinct, safety-scoped surface, deliberately not folded into the guided-session experience it sits beside) |
| _(`sr-lib-` not claimed)_ | SR-418 (`PASS-dashboard-panel-and-carousel.md` Part F1) found `dashboard.html` already has a track panel + stream — `.sr-dash-rail`/`.sr-dash-railbtn` + `.sr-dash-carwrap`, both under `sr-dash-`, not new construction. F1 adapted those existing classes (borderless treatment, `.sr-dash-carbtn`) rather than porting the mockup's `.tracks`/`.trk` under a new `sr-lib-` code, which would have forked the architecture in two places for one feature. If F4 ever needs this panel on `protocol.html`, extend `sr-dash-rail`/`sr-dash-carwrap` there rather than claiming `sr-lib-` for a second implementation. |
